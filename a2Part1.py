...
def calculate_acc(predictions, actual_onehot_encoded, instance_length):
...
if __name__ == '__main__':
    ...
    n_hidden = 2
    n_out = 3
    learning_rate = 0.2  # 0.8 show best performance 
    ...
    nn = Neural_Network(n_in, n_hidden, n_out, initial_hidden_layer_weights, initial_output_layer_weights,
                        learning_rate, initial_hidden_bias, initial_output_bias)
    ...
    test_accuracy = calculate_acc(test_predict_class, test_onehot_encoded , len(test_instances))
    print("The Accuracy test data is: {:.2f} %".format(test_accuracy*100))
